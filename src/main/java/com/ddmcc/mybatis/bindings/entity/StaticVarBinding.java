...
public class StaticVarBinding extends BaseBinding {
    ...
    public List<VarDeclSqlNode> buildVarDeclSqlNode() {
        ...
        StringBuilder builder = StringBuilderHolder.get().append(StringPool.AT).append(type.getCanonicalName()).append(StringPool.AT);
        int count = builder.length();
        Stream.of(varName).forEach(name -> {
            String string = builder.append(name).toString();
            varDeclSqlNodes.add(new VarDeclSqlNode(StringUtils.join(getSimpleName(type.getCanonicalName()), StringPool.DOT, name), string));
            if (StringUtils.isNotBlank(alias)) {
                varDeclSqlNodes.add(new VarDeclSqlNode(StringUtils.join(alias, StringPool.DOT, name), string));
            }
